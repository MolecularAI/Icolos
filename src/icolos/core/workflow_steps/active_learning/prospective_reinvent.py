...
from icolos.core.workflow_steps.active_learning.acquisition_function import AcquisitionFunction
...
class StepProspectiveREINVENT(StepBase):
    ...
    def _run(self, original_smiles: list, save_path: str):
        ...
        epochs, warmup, retrain, initial_pooling_epochs, acquisition_batch_size, \
            acquisition_function_name, surrogate_model_type, oracle_config = self._get_run_parameters()
        ...
        self._logger.log(
            ...
            f"Oracle Config: {oracle_config}", _LE.INFO
        )
        ...
        if curr_epoch <= warmup:
            ...
            self._run_normal_reinvent(curr_epoch=curr_epoch, save_path=save_path,
                                      original_smiles=original_smiles, oracle_config=oracle_config)
            ...
            self._run_normal_reinvent_with_pooling(curr_epoch=curr_epoch, save_path=save_path,
                                                   original_smiles=original_smiles, oracle_config=oracle_config)

        # run active learning REINVENT for the remainder of the experiment
        else:
            self._run_active_learning_reinvent(curr_epoch=curr_epoch, save_path=save_path,
                                               original_smiles=original_smiles,
                                               oracle_config=oracle_config, warmup=warmup,
                                               initial_pooling_epochs=initial_pooling_epochs,
                                               retrain=retrain, surrogate_model_type=surrogate_model_type,
    ...
    def morgan_fingerprints(self, smiles) -> list:
        ...
        return [Chem.AllChem.GetMorganFingerprintAsBitVect(mol, radius=3, nBits=2048) for mol in molecules]
    ...
    def _save_state(self, curr_epoch, save_path: str, original_smiles, labels, pooled_smiles=None, pooled_labels=None,
                    surrogate=None, save_pool=False, non_acquired_smiles=None, non_acquired_labels=None
                    ):
        ...
        if save_pool:
            ...
            df = pd.DataFrame({"smiles": pooled_smiles,
        ...
        if non_acquired_smiles is not None:
            df = pd.DataFrame({"smiles": non_acquired_smiles,
                               "labels": non_acquired_labels})
            df.to_csv(os.path.join(save_path, f"predicted_smiles/epoch_{curr_epoch}.csv"))

        # save the surrogate model state
        if isinstance(surrogate, SurrogateModel):
            pkl_path = os.path.join(save_path, f"surrogate_states/epoch_{curr_epoch}.sav")
    ...
    def _read_state(self, curr_epoch, save_path: str):
        ...
        try:
            ...
            df = pd.read_csv(os.path.join(save_path, f"pooled_smiles/epoch_{curr_epoch-1}.csv"))
        ...
        try:
            surrogate = pickle.load(open(os.path.join(save_path, f"surrogate_states/epoch_{curr_epoch-1}.sav"), "rb"))
        except Exception:
            surrogate = None
    ...
    def _query_oracle(self, save_path: str, original_smiles, oracle_config) -> list:
        ...
        self._create_manipulable_compounds_file(original_smiles=original_smiles, save_path=save_path)
        oracle_workflow.execute()

        # extract the best scores per compound and its enumerations
        scores = self._extract_scores_from_oracle(original_smiles=original_smiles, oracle_workflow=oracle_workflow)
        ...
        for smiles, compound in zip(original_smiles, oracle_workflow.get_steps()[-1].get_compounds()):
            ...
            for enumeration in compound.get_enumerations():
                for conformer in enumeration.get_conformers():
                    ...
                    score = float(conformer.get_molecule().GetProp(_SGE.GLIDE_DOCKING_SCORE))
    ...
    def _write_reinvent_feedback(self, original_smiles, labels):
        ...
        output_dict = {"results": [{"values_key": values_key, "values": labels}],
                       "names": [idx for idx in range(len(original_smiles))]}

        with open(path, "w+") as f:
            json.dump(output_dict, f, indent=2)

    def _run_normal_reinvent(self, curr_epoch, save_path, original_smiles, oracle_config):
        """runs a normal REINVENT epoch"""
        scores = self._query_oracle(save_path=save_path, original_smiles=original_smiles, oracle_config=oracle_config)

        # TODO: bottom doesn't store the compounds' enumeration SMILES - should we do this?
        self._save_state(curr_epoch=curr_epoch, original_smiles=original_smiles, labels=scores, save_path=save_path)
        self._write_reinvent_feedback(original_smiles=original_smiles, labels=scores)

    def _run_normal_reinvent_with_pooling(self, curr_epoch, save_path: str, original_smiles, oracle_config):
        """runs a normal REINVENT epoch and stores all (SMILES, scores) pairs as training data for the surrogate"""
        # read the current state
        pooled_smiles, pooled_labels, surrogate = self._read_state(curr_epoch=curr_epoch, save_path=save_path)

        scores = self._query_oracle(save_path=save_path, original_smiles=original_smiles, oracle_config=oracle_config)

        self._save_state(curr_epoch=curr_epoch, original_smiles=original_smiles, labels=scores,
                         pooled_smiles=pooled_smiles, pooled_labels=pooled_labels, save_pool=True,
                         save_path=save_path)

        self._write_reinvent_feedback(original_smiles=original_smiles, labels=scores)

    def _run_active_learning_reinvent(self, curr_epoch, save_path: str, original_smiles, oracle_config,
                                      warmup, initial_pooling_epochs, retrain, surrogate_model_type,
        ...
        acquisition_function = AcquisitionFunction(surrogate=surrogate,
                                                   function=acquisition_function_name,
                                                   acquisition_batch_size=acquisition_batch_size)
        # select points to acquire based on acquisition function
        acquired_smiles, non_acquired_smiles = acquisition_function.partition_compounds(original_smiles=original_smiles)

        # send the acquired compounds to the oracle
        acquired_labels = self._query_oracle(save_path=save_path, original_smiles=acquired_smiles, oracle_config=oracle_config)
        # predict the non-acquired compounds and cast to type list from np.array for compatibility with other methods
        non_acquired_labels = list(surrogate.predict(self.morgan_fingerprints(smiles=non_acquired_smiles)))
        ...
        self._save_state(curr_epoch=curr_epoch, save_path=save_path, original_smiles=acquired_smiles,
                         labels=acquired_labels, pooled_smiles=pooled_smiles,
                         pooled_labels=pooled_labels, non_acquired_smiles=non_acquired_smiles,
                         non_acquired_labels=non_acquired_labels, surrogate=surrogate, save_pool=True
                         )
        ...
        self._write_reinvent_feedback(original_smiles=concatenated_smiles, labels=concatenated_labels)
    ...
    def _get_run_parameters(self):
        ...
        initial_pooling_epochs = self._get_additional_setting(_SALE.INITIAL_POOLING_EPOCHS, default=5)
        acquisition_batch_size = self._get_additional_setting(_SALE.ACQUISITION_BATCH_SIZE, default=40)
        acquisition_function = self._get_additional_setting(_SALE.ACQUISITION_FUNCTION, default=_SALE.RANDOM)
        surrogate_model_type = self._get_additional_setting(_SALE.SURROGATE_MODEL_TYPE, default=_SALE.RANDOM_FOREST_REGRESSOR)
        oracle_config = self._get_additional_setting(_SALE.ORACLE_CONFIG)

        return epochs, warmup, retrain, initial_pooling_epochs, acquisition_batch_size, \
            acquisition_function, surrogate_model_type, oracle_config
    ...
    def execute(self):
        ...
        original_smiles = [compound.get_enumerations()[0].get_original_smile() for compound in self.data.compounds]
